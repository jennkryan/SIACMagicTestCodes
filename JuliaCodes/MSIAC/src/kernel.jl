...
function kernel_coefficients(T::matD, zw::GZW)
	...
	for p = 1:m
		for g = 1:m
			#TT[g,end] -= 1.e-15
			spline  = BS.BSplineBasis(n-1, TT[g,:])[n-1]
			for k = 1:n-1
...
function evaluate_kernel(T::matD, c::vecD, z::vecD)
	...
	for j = 1:m
		#TT[j,end] -= 1.e-15
		sp = BS.splinevalue.(BS.BSplineBasis(n-1, TT[j,:])[n-1], z)
        val .+= sp .* c[j]
	end
	return val
end
...
function plot_kernel(T::matD, c::vecD)
    ...
    for j = 1:m
		local bs     = BS.BSplineBasis(n-1, TT[j,:])
        splines[j,:] = BS.splinevalue.(bs[n-1], x)
		@info " J $j -- KNOTS $(TT[j,:])  SPLINE $bs "
		for k = 1:length(x)
			@info "  $(x[k]) --> VALS $(splines[j,k])"
		end
