function out = nonUniformPostShift( elements, boundaries, BSorder, RS, kernelScale, shift )
...
numElements = numElements - 2*shift;
kapprox = order -1;
...
for i = 1:numElements
    h = boundaries( shift + i + 1 ) - boundaries( shift + i  );
    out(1, offset:last) = h * 0.5 * (zEval' + 1) + boundaries(shift + i );
    offset = last + 1;
    last = offset + evalPoints-1;
end
%%%%%%%%%%%
...
if kernelScale ==0
    for n=1:numElements
        kernelScale = max(kernelScale,boundaries( shift + n + 2 ) - boundaries( shift + n +1 ));
        elementWidth(n) = boundaries( shift + n + 2 ) - boundaries( shift + n +1 );
    end
end
%%%%%%%%%%%
...
for n = 1:numElements
    elementWidth = boundaries( shift + n + 2 ) - boundaries( shift + n +1 );
    for xIndex = 1:evalPoints
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        uhatBreaks = funcBreakPoints_periodicShift( boundaries, localCompactSupport, shift );  % in world coordinates (outside of interval definition)
        ...
        for i = 1:(size(breaks,2) - 1)
            ...
            uhatVal = periodicEvalShift( elements, boundaries, quadZ, shift );
